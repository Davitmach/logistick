...
if(active == false && click == true) {
    ...
    return(
        ...
        <div>
            <div className='img w-full h-[203px] bg-white rounded-[8px]'></div>
...
Через две недели рояль благополучно прибыл в пункт назначения, а клиент оставил восторженный отзыв. Это ещё раз доказало: надежные грузоперевозки по России — это не только скорость, но и забота о каждом заказе.  `}/>
                <News_box  text={'Секреты быстрой доставки: как мы сократили время перевозки на 30%'} description={`В мире грузоперевозок время — это деньги. Мы понимаем, как важно для наших клиентов получать грузы вовремя, поэтому постоянно работаем над оптимизацией процессов.  
...
Если вам нужны быстрые и надежные грузоперевозки по России, вы знаете, куда обратиться!  `}/>
                <News_box  text={'Зимние грузоперевозки: как мы справляемся с суровыми условиями'} description={`Зима в России — это не только снег и мороз, но и дополнительные сложности для грузоперевозок. Однако наша компания готова к любым вызовам!  
